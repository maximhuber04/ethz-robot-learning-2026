...
def reset_robot(default_qpos: np.ndarray) -> np.ndarray:
    """
    DONE: Implement robot reset to its default joint positions with some small uniform noise (-0.5, 0.5).
...
def reset_target_position(base_pos: np.ndarray) -> np.ndarray:
    """
    DONE: Sample and compute a new random target position relative to the base from uniform distribution.
    ...
    target_offset = np.random.uniform(lower, upper)
...
def process_action(action: np.ndarray, jnt_range: np.ndarray) -> np.ndarray:
    """
    DONE: Convert normalized actions [-1, 1] to target joint positions.
    ...
    jnt_min = jnt_range[:, 0]
    jnt_max = jnt_range[:, 1]
    target_qpos = jnt_min + 0.5 * (action_clipped + 1.0) * (jnt_max - jnt_min)
    return target_qpos
...
def get_obs(
    ...
    """
    DONE: Extract the observation vector from the environment robot state variables.
    ...
    base_rot_w_as_quat = rot_mat_to_quat(base_rot_w)
    ee_rot_w_as_quat = rot_mat_to_quat(ee_rot_w)
    ee_rot_base_as_quat = quat_mul(quat_conjugate(base_rot_w_as_quat), ee_rot_w_as_quat)
    ee_quat_base = quat_normalize(ee_rot_base_as_quat)
