...
def build_keypoints(count=16, width=0.25, x_offset=0.3, z_offset=0.25):
    ...
    x = np.full_like(y, x_offset, dtype=float)
...
def ik_track(
    ...
    for i in range(max_iters):
        ...
        # DONE: compute end-effector position error
        err_pos = target_pos - data.site(site_name).xpos

        # DONE: check if the 2-norm of the position error is within a small threshold (1e-3), if yes, break the loop
